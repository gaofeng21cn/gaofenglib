...
calc_oncotypedx_crc <- function (dat)
{
  ...
  stroma_sc <- apply(ge.onco[na.omit(match(stroma, rownames(ge.onco))), ], 2, mean)
  CC_sc <- apply(ge.onco[na.omit(match(CC, rownames(ge.onco))), ], 2, mean)
  Individual_sc <- as.numeric(as.vector(ge.onco[na.omit(match(Individual, rownames(ge.onco))),
                                                ]))
  RF_sc <- apply(ge.onco[na.omit(match(RF, rownames(ge.onco))), ], 2, mean)
  corrected_stroma <- stroma_sc - RF_sc + 10
  corrected_CC <- CC_sc - RF_sc + 10
  corrected_individual <- Individual_sc - RF_sc + 10
  RS_score <- 0.15 * corrected_stroma - 0.3 * corrected_CC +
    0.15 * corrected_individual
  oncotypeDX_score <- 44 * (RS_score + 0.82)
  oncotypeDX_class <- cut(oncotypeDX_score, breaks = c(-Inf,
                                                       30, 41, Inf), labels = c("Low", "Intermediate", "High"))

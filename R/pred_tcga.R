...
extract_tcga_clinical <- function(clin, ESAC = FALSE, STAD = FALSE)
{
  ...
  if(ESAC){
    age <- clin[, "patient.primary_pathology.age_at_initial_pathologic_diagnosis"]} else{
  ...
  ## relapse
  if(ESAC){
    labels <- clin[, "patient.new_tumor_events.new_tumor_event_after_initial_treatment"]} else if(STAD){
  ...
  if(ESAC){
    t <- c("patient.new_tumor_events.new_tumor_event.days_to_new_tumor_event_after_initial_treatment",
           ...
           "patient.follow_ups.follow_up.days_to_last_followup")} else if(STAD){
  ...
  ## os time
  if(ESAC){
    t <- c("patient.days_to_death", "patient.follow_ups.follow_up.days_to_last_followup")} else{
  ...
  if(ESAC) {
    type <- factor(clin[, "patient.biospecimen_cqcf.histological_type"] == "esophagus adenocarcinoma  nos",
                   levels = c(T, F), labels = c("EAC", "ESCC"))
    location <- clin[, "patient.primary_pathology.esophageal_tumor_cental_location"]
    data.frame(age, gender, type, location, stage, LN, Tstage, DFS, relapse, relapse.time, death, death.time)
  } else{
    data.frame(age, gender, stage, LN, Tstage, DFS, relapse, relapse.time, death, death.time, chemo, chemo_resp)
  }
}
